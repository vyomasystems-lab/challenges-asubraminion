...
async def test2_mux(dut):
    ...
    for i in range(200):
        ...
        #print(inputt[SEL])
        #print(dut.out.value)
        assert dut.out.value==inputt[SEL], "Randomised test 2 failed, for SEL={A}, output should be {B} (decimal), but is found to be {C} (binary)".format(A=SEL, B=inputt[SEL], C=dut.out.value)
